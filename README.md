...
| 강사진 | 이름/역할(여러 개 가능)/소개/사진 (사진 없으면 이니셜 아바타, 사진은 업로드 후 원 안에서 위치·확대 조정 가능, PC 화면에서는 카드 너비에 거의 맞춘 큰 원형 사진으로 표시) |
...
| 캠퍼스 사진 | 히어로/프로그램/강사진 섹션 다음 3곳에 화면 전체 너비로 크게 들어가는 대학 캠퍼스 사진. 각 자리마다 사진·대학명·위치·문구를 따로 수정하고 "표시(visible)/숨김(invisible)"을 각각 켤 수 있음 |
...
### 캠퍼스 사진 (섹션 사이 큰 사진 밴드)

하버드·예일·스탠포드처럼 학생들이 진학한(또는 목표로 하는) 대학 캠퍼스
사진을 홈페이지 섹션 사이사이에 화면 전체 너비로 크게 보여주는 기능입니다.
자유롭게 몇 개든 추가할 수 있는 목록이 아니라, **위치가 3곳으로 고정된
슬롯**입니다 (`app/page.tsx` 기준):

1. 히어로 섹션 바로 다음
2. 프로그램 트랙 섹션 다음
3. 강사진 섹션 다음

관리자 → **캠퍼스 사진**에서 이 3곳 각각의 사진을 교체하고, 대학명·위치
·문구(사진 위에 작게 표시되는 한 줄 설명)를 수정할 수 있습니다. 각 자리마다
**"표시(visible)" 체크박스**가 따로 있어서, 체크를 끄면(invisible) 그
자리는 사이트에서 바로 숨겨집니다 — 아직 사진을 올리지 않은 자리도 마찬가지로
자동으로 숨겨지므로(빈 회색 상자나 빈 공간이 남지 않음), 3곳을 전부 채우지
않아도 안전합니다. 최대 4MB까지 업로드 가능하며, 가로로 넓은 사진일수록 잘
어울립니다.

세 자리 모두 미리 이름/위치/문구가 채워져 있지만(하버드/예일/스탠포드)
사진은 비어 있는 상태로 시작하므로, 실제로 쓰려면 관리자에서 사진만
올리면 됩니다. 위치를 3곳보다 더 늘리고 싶거나 자유롭게 추가/삭제할 수
있는 목록으로 바꾸고 싶다면 요청해주세요 — 지금은 의도적으로 고정된 3곳만
지원합니다.
